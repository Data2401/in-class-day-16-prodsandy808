...
```{r}
heights %>% summarise(mean(height))

heights %>% summarise(median(height))
```
...
```{R}
heights %>% filter(sex == "Female") %>% summarise(tallest_woman = max(height))

heights %>% filter(sex == "Male") %>% summarise(shortest_man = min(height))
...
```{R}
##heights %>% group_by(sex) %>% summarise(mean_heights = mean(height)) %>% ggplot(heights, aes(x = mean_heights, y = sex)) + geom_bar()
